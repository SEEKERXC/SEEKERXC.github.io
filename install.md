...
这些环境变量被传递给${LEADERBOARD_ROOT}/leaderboard/leaderboard\_evaluator.py，它作为执行模拟的入口。可以参考leaderboard\_evaluator.py，了解更多关于你的agent将如何被执行和评估的细节。
...
你也可以访问ego agent为到达目的地所应走的路线。使用self.\_global_plan成员来访问地理定位路线，使用self.\_global\_plan\_world\_coord来访问其在世界位置的对应。
